...
int loadOrdersFromFile(Order *orders, const char *filename) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        return 0; // No existing orders
    }
    int numOrders = 0;
    while (fscanf(file, "Order ID: %d\n", &orders[numOrders].Id) == 1) {
        for (int j = 0; j < MAX_PRODUCTS; j++) {
            if (fscanf(file, "Product #%*d: %49[^-] - Price: %f - Quantity: %d\n",
                       orders[numOrders].Products[j].Name,
                       &orders[numOrders].Products[j].Price,
                       &orders[numOrders].Products[j].Quantity) != 3) {
                orders[numOrders].Products[j].Quantity = 0;
                break;
            }
        }
        fscanf(file, "Total: %f\n\n", &orders[numOrders].Total);
        numOrders++;
    }
    fclose(file);
    return numOrders;
...
void createNewOrder(Order *orders, int *numOrders, int *nextId) {
    int id = *numOrders;
...
int main() {
    ...
    numOrders = loadOrdersFromFile(orders, filename);
    int nextId = numOrders > 0 ? orders[numOrders - 1].Id + 1 : 0;
    ...
    do {
        ...
        switch (choice) {
            case 1:
                createNewOrder(orders, &numOrders, &nextId);
                break;
            case 2:
